...
module.exports = dims = async (dims, m, chatUpdate, store) => {
try {
...
const botNumber = await dims.decodeJid(dims.user.id);
const sender = m.key.fromMe ? (dims.user.id.split(':')[0] + '@s.whatsapp.net' || dims.user.id) : (m.key.participant || m.key.remoteJid);
const senderNumber = sender.split('@')[0];
const isBot = botNumber.includes(senderNumber);
const itsMeYumi = m.sender == botNumber ? true : false
const DimsIsCreator = (m && m.sender && [botNumber, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)) || false;
...
const groupMetadata = m.isGroup ? await dims.groupMetadata(m.chat).catch(e => {}) : {};
...
//dims.sendPresenceUpdate(ngetik, iniFrom)
//dims.readMessages([m.key])
//dims.sendPresenceUpdate('available', iniFrom)
console.log(color(`\n< ======================== >\n`, 'cyan'))
...
if (isCmd && !DimsIsCreator) antiSpam.addFilter(from)
...
await dims.sendMessage(m.chat, { react: { text: react[i], key: m.key }})
}}
...
/** pp user **/
try {
ppuser = await dims.profilePictureUrl(m.sender, 'image')
} catch (err) {
ppuser = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
}
ppnyauser = await dims.resize(ppuser, 300, 300)
...
return dims.ev.emit('messages.upsert', { messages: [emit_msg], type: 'notify' })
}
}
}
...
dims.sendMessage(iniFrom, { text: sendResponList(iniFrom, body.toLowerCase(), db_respon_list) }, { quoted: m })
} else {
dims.sendMessage(iniFrom, { image: await getBuffer(get_data_respon.image_url), caption: get_data_respon.response }, { quoted: m })
} 
}
...
let messages = await generateWAMessage(m.chat, { text: text, mentions: m.mentionedJid }, { userJid: dims.user.id, quoted: m.quoted && m.quoted.fakeObj })
messages.key.fromMe = areJidsSameUser(m.sender, dims.user.id)
...
if (chat && chat.isBanned && !DimsIsCreator) return
}
...
dims.sendMessage(m.chat, { react: { text: '❌️', key: m.key }})
if (!isBotAdmins) return
let gclink = (`https://chat.whatsapp.com/` + await dims.groupInviteCode(m.chat))
let isLinkThisGc = new RegExp(gclink, 'i')
let isgclink = isLinkThisGc.test(m.text)
if (isgclink && DimsIsCreator && isAdmins) return
dims.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
}}
...
let isgclink = isLinkThisGc.test(m.text)
if (isgclink && DimsIsCreator && isAdmins) return
dims.sendMessage(m.chat, { delete: m.key })
}}

// ===============< memasuki area case >=============== //
switch(command) {
case 'allmenu':{
dims.sendMessage(m.chat, { react: { text: '🕒', key: m.key }})
...
// ===============< owner >=============== //
case 'upsw': {
if (!DimsIsCreator) return m.reply(mess.owner)
...
if (mime) {
if (/audio/.test(mime)) {
await dims.sendMessage('status@broadcast', { audio: await quoted.download(), mimetype: 'audio/mp4', ptt: true, waveform: [100, 0, 100, 0, 100, 0, 100] }, { backgroundColor: colors[Math.floor(Math.random() * colors.length)], statusJidList });
...
await dims.sendMessage('status@broadcast', { [type]: await quoted.download(), caption: text || '' }, { statusJidList });
await m.reply(`Up status ke : ${statusJidList.length} Kontak`);
}
} else {
if (!text) return m.reply('Mana text?')
await dims.sendMessage('status@broadcast', { text: text }, { backgroundColor: colors[Math.floor(Math.random() * colors.length)], textArgb: 0xffffffff, font: fonts[Math.floor(Math.random() * colors.length)], statusJidList });
await m.reply(`Up status ke : ${statusJidList.length} Kontak`);
}
}
break
...
await dims.groupAcceptInvite(result).then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
}
break
// ===================================== //
case 'out': {
if (!DimsIsCreator) return m.reply(mess.owner)
m.reply('Sayonaraaa...')
await dims.groupLeave(m.chat).then((res) => m.reply(jsonformat(res))).catch((err) => m.reply(jsonformat(err)))
}
break
// ===================================== //
case 'addprem':
if (!DimsIsCreator) return m.reply(mess.owner)
if (!q) return reply(`Masukan Nomornya contoh: ${prefix + command} 6281398274790`)
...
dims.sendMessage(m.chat, { react: { text: '🕒', key: m.key }})
var medis = await dims.downloadAndSaveMediaMessage(quoted, 'ppbot.jpeg')
if (args[0] == `/panjang`) {
var { img } = await generateProfilePicture(medis)
await dims.query({ tag: 'iq', attrs: { to: botNumber, type:'set', xmlns: 'w:profile:picture' }, content: [{ tag: 'picture', attrs: { type: 'image' }, content: img }] })
fs.unlinkSync(medis)
m.reply(`Success`)
} else {
var memeg = await dims.updateProfilePicture(botNumber, { url: medis })
fs.unlinkSync(medis)
m.reply(`Success`)
}
}
...
let encmedia = await dims.sendImageAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return m.reply('Maksimal 10 detik!')
let media = await quoted.download()
let encmedia = await dims.sendVideoAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
let dwnld = await dims.downloadAndSaveMediaMessage(qmsg)
let fatGans = await TelegraPH(dwnld)
let smeme = `https://api.memegen.link/images/custom/${encodeURIComponent(bawah)}/${encodeURIComponent(atas)}.png?background=${fatGans}`
let FaTiH = await dims.sendImageAsSticker(m.chat, smeme, m, { packname: global.packname, author: global.auhor })
...
if (/image/.test(mime)) {
let media = await dims.downloadMediaMessage(qmsg)
let encmedia = await dims.sendImageAsSticker(m.chat, media, m, { packname: teks1, author: teks2 })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return m.reply('Maksimal 10 detik!')
let media = await dims.downloadMediaMessage(qmsg)
let encmedia = await dims.sendVideoAsSticker(m.chat, media, m, { packname: teks1, author: teks2 })
await fs.unlinkSync(encmedia)
} else {
return m.reply(`Kirim Gambar/Video Dengan Caption ${prefix + command}\nDurasi Video 1-9 Detik`)
}
...
if (stiker) dims.sendFile(m.chat, stiker, 'wms.webp', '', m, false, { asSticker: true })
...
dims.sendImageAsSticker(m.chat, rest.result, m, { packname: `${global.packname}`, author: `${global.author}`})
}
break
// ===============< info >=============== //
case 'totalfitur':{
...
const sentMsg = await dims.sendContactArray(m.chat, [
  ['6281398274790', 'Im-Dims??', 'Name', 'Number', 'dev@ssateam.my.id', 'Japanese', 'https://github.com/Im-Dims', 'Ini sebenarnya buat apa ya??']
], m)
await dims.sendTextWithMentions(m.chat, `Hai kak @${m.sender.split('@')[0]} itu adalah nomer ownerku, ga boleh di spam ya ~`, m)
}
break
// ===================================== //
case 'listpc': {
...
dims.sendTextWithMentions(m.chat, teks, m)
}
break
// ===================================== //
case 'listgc': {
...
let metadata = await dims.groupMetadata(i)
...
dims.sendMessage(m.chat, { image: { url: 'https://telegra.ph/file/19159af95476d12c66bde.jpg' }, caption: `Hai Kak @${sender.split("@")[0]} Mau ${command}?
...
dims.sendMessage(iniFrom, { image: { url: 'https://telegra.ph/file/0c62e2a3dff3bbf57f594.png' }, caption: `Hai Kak @${sender.split("@")[0]} Mau ${command} Ya?

Harganya Bisa Pencet Link Di Bawah Ya Kak
https://wa.me/p/8363158480377147/62895385006567
...
dims.sendMessage(m.chat, { audio: Buffer.from(a.audioUrl.split("base64,")[1],"base64"), mimetype: "audio/mpeg" }, { quoted : m })
}
break
// ===================================== //
...
dims.sendMessage(m.chat, { forward: q }, { quoted: m })
}
break
// ===================================== //
case 'tr':{
...
if (data.img) return dims.sendMessage(m.chat, { image: data.img, caption: data.desc }, { quoted: m })
...
if (!/text|json/.test(res.headers.get('content-type'))) return dims.sendFile(m.chat, url, 'file', `_iyh_`, m)
...
let img = await dims.downloadAndSaveMediaMessage(q)
...
dims.sendMessage(m.chat,{image: krt.result, caption: global.mess.success },{quoted:m})
}
break
// ===================================== //
case 'up': case 'upload':{
...
let img = await q.download()
if (!img) return m.reply('not detected')
let iyh = await dims.downloadAndSaveMediaMessage(q)
...
dims.sendFile(m.chat, This, 'remini.jpg', global.mess.success, m);
}
break
// ===============< main >=============== //
case 'disk':{
...
dims.send2ButtDoc(m.chat, '', mzd, global.wm, global.botname, global.media.sig, 'https://telegra.ph/file/137a5210e0723261d0d8b.jpg', 'Instagram', global.media.sig, 'MENU', `${prefix}menu`, m)
} else {
addRegisteredUser(m.sender, namaUser, umurUser, serialUser)
adduang(sender)
adduangUser(sender, 1000)
...
dims.sendMessage(m.chat, { video: { url: data.nowm }, caption: data.title, fileName: `tiktok.mp4`, mimetype: 'video/mp4' }).then(() => {
dims.sendMessage(m.chat, { audio: { url: data.audio }, fileName: `tiktok.mp3`, mimetype: 'audio/mp3' });
});
} catch (e) {
...
await dims.sendFile(m.chat, urls[i], 'file', `${i + 1}/${totalCount}`, m);
}
} else {
...
const { data } = await axios.post("https://allvideodownloader.cc/wp-json/aio-dl/video-data/", { url: text });
await dims.sendMessage(m.chat, { video: { url: data.medias[0].url }, caption: global.mess.success, fileName: `tiktok.mp4`, mimetype: 'video/mp4' })
}
break;
// ===================================== //
case 'ttslide':{
...
await dims.sendMessage(m.chat, { image: { url: urls[i] }, caption: `(${i + 1}/${totalCount})` }, { quoted: m });
}} else {
...
if (mediaType === 'video') {
dims.sendMessage(m.chat, { video: { url: res.result.media} , caption: res.result.title }, { quoted: m })
} else if (mediaType === 'image') {
dims.sendMessage(m.chat, { image: { url: res.result.media}, caption: res.result.title }, { quoted: m })
} else {
...
dims.sendMessage(m.chat, { video: { url: res.result.no_watermark }, caption: res.result.title, fileName: `tiktok.mp4`, mimetype: 'video/mp4' }).then(() => {
dims.sendMessage(m.chat, { audio: { url: res.result.music }, fileName: `tiktok.mp3`, mimetype: 'audio/mp4' })
})
}
break;
// ===================================== //
...
dims.sendMessage(m.chat,{image: {url: getRandom(res)}, caption: global.mess.success },{quoted:m})
}
break
// ===================================== //
case 'play':{
...
dims.sendMessage(m.chat, { audio: buffer, mimetype: "audio/mpeg", }, { quoted: m })
...
if (/audio/.test(mime)) {
let media = await dims.downloadAndSaveMediaMessage(qmsg)
let ran = pickRandom('.mp3')
exec(`ffmpeg -i ${media} ${set} ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(media)
if (err) return m.reply(err)
let buff = fs.readFileSync(ran)
dims.sendMessage(m.chat, { audio: buff, mimetype: 'audio/mpeg' }, { quoted : m })
fs.unlinkSync(ran)
})
...
if (!isAdmins && !isGroupOwner && !DimsIsCreator) return m.reply(mess.admin)
if (!isBotAdmins) return m.reply(mess.botAdmin)
if (!text && !m.quoted) m.reply('masukkan nomor yang ingin di tambahkan')
let users = m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await dims.groupParticipantsUpdate(m.chat, [users], 'add').catch(console.log)
}
break
// ===================================== //
case 'kick':{
...
let users = m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await dims.groupParticipantsUpdate(m.chat, [users], 'remove').catch(console.log)
}
break
// ===================================== //
case 'promote':{
...
let users = m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await dims.groupParticipantsUpdate(m.chat, [users], 'promote').catch(console.log)
}
break
// ===================================== //
case 'demote':{
...
let users = m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await dims.groupParticipantsUpdate(m.chat, [users], 'demote').catch(console.log)
}
break
// ===================================== //
case 'group': { 
...
if (!isBotAdmins) return m.reply(mess.botAdmin)
if (args[0] === 'close'){
await dims.groupSettingUpdate(m.chat, 'announcement').then((res) => m.reply(`Sukses Menutup Group`)).catch((err) => m.reply(jsonformat(err)))
} else if (args[0] === 'open'){
await dims.groupSettingUpdate(m.chat, 'not_announcement').then((res) => m.reply(`Sukses Membuka Group`)).catch((err) => m.reply(jsonformat(err)))
} else {
...
if (!isBotAdmins) return m.reply(mess.botAdmin)
 if (args[0] === 'open'){
await dims.groupSettingUpdate(m.chat, 'unlocked').then((res) => m.reply(`Sukses Membuka Edit Info Group`)).catch((err) => m.reply(jsonformat(err)))
} else if (args[0] === 'close'){
await dims.groupSettingUpdate(m.chat, 'locked').then((res) => m.reply(`Sukses Menutup Edit Info Group`)).catch((err) => m.reply(jsonformat(err)))
} else {
m.reply(`Silahkan Ketik ${prefix + command} open/ ${prefix + command} close`)
}
}
...
dims.sendMessage(m.chat, { text : q ? q : '' , mentions: participants.map(a => a.id)}, {quoted:m})
}
break
// ===================================== //
case 'editsubjek': {
...
await dims.groupUpdateSubject(m.chat, text).then((res)).catch((err) => m.reply(jsonformat(err)))
}
break
// ===================================== //
case 'editdesk':{
...
if (!isBotAdmins) return m.reply(mess.botAdmin)
if (!text) return m.reply('Text Nya ?')
await dims.groupUpdateDescription(m.chat, text).then((res)).catch((err) => m.reply(jsonformat(err)))
}
break
// ===================================== //
case 'setppgc': case 'setppgroup': case 'setgcpp': case 'setgrouppp': {
if (!m.isGroup) return m.reply(mess.group)
if (!isAdmins && !DimsIsCreator) return m.reply(mess.admin)
if (!isBotAdmins) return m.reply(mess.botAdmin)
if (!quoted) return m.reply(`Where is the picture?`)
if (!/image/.test(mime)) return m.reply(`Send/Reply Image With Caption ${prefix + command}`)
if (/webp/.test(mime)) return m.reply(`Send/Reply Image With Caption ${prefix + command}`)
var mediz = await dims.downloadAndSaveMediaMessage(quoted, 'ppgc.jpeg')
if (args[0] == `/panjang`) {
...
var memeg = await dims.updateProfilePicture(m.chat, { url: mediz })
fs.unlinkSync(mediz)
m.reply(`Success`)
}
}
...
dims.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted:m })
}
break
// ===================================== //
case 'on': case 'enable':{
...
dims.kirimListWoy(m.chat, 'Hai kak ' + pushname + ' silakan di pilih opsi berikut ini', 'Klik button di bawah ya', global.wm, data, m)
}
break
...
case 'shadow-sky':
case 'flaming':{
if (!text) return m.reply(`Ex: ${prefix + command} Dims`)
dims.sendMessage(m.chat, { react: { text: '🕒', key: m.key }})
dims.sendMessage(m.chat,{image: {url: `https://dsgner.vercel.app/api/photooxy/${command}?text=${text}`}, caption: global.mess.success },{quoted:m})
}
break;
// ===================================== //
case 'pubg':{
let t = text.split('|');
if (t.length < 2) return m.reply(`Ex: ${prefix + command} Dims|Sukaloli`);
let text1 = t[0];
let text2 = t[1];
dims.sendMessage(m.chat, { react: { text: '🕒', key: m.key }})
dims.sendMessage(m.chat,{image: {url: `https://dsgner.vercel.app/api/textpro/${command}?text=${text1}&text2=${text2}`}, caption: global.mess.success },{quoted:m})
}
break
// ===============< textpro >=============== //
case 'chocolate-cake':
...
if (!text) return m.reply(`Ex: ${prefix + command} Dims`)
dims.sendMessage(m.chat, { react: { text: '🕒', key: m.key }})
dims.sendMessage(m.chat, { image: { url: `https://dsgner.vercel.app/api/textpro/${command}?text=${text}` }, caption: global.mess.success }, { quoted: m })
}
break;
// ===================================== //
case 'marvel-studios':
...
// ===================================== //
case 'tqto':
const ckck = `Big Thanks To

*-* Putu
*-* SSA Team
*-* Dims (my self)

Dan kalian semua yang sudah menggunakan script ini:)`
m.reply(ckck)
break
// ===================================== //
case 'setmenu':
...
dims.kirimListWoy(m.chat, '*CHANGE MENU*', 'Hai kak ' + pushname + ' silakan di pilih opsi berikut ini', global.wm, data, m)
} else if (!m.isGroup){
let sections = [{
title: 'CHANGE BOT MENU',
rows: [
...
dims.sendListMsg(m.chat, 'Pilih salah satu mode menu di bawah', global.wm, '*CHANGE MENU*', 'Click Here', sections, m)
}
}
break
// ===================================== //
case 'help': case 'menu': {
if (!Belomdaftar) return m.reply(belomregis) & dims.kirimBeton(m.sender, daftar1, daftar2, 'Daftar', `${prefix}reg`, m) 
if (isBan) return m.reply(mess.banned)
let user = global.db.data.users[m.sender]
...
● Status : ${DimsIsCreator ? 'Developer' : 'User'}
...
rows: [
{title: "「ღ」| Tqto", description: "Credits script bot ini", id: `${prefix}tqto`},
{title: "「ღ」| Kosong:v", description: "Menampilkan menu kosong:v", id: `${prefix}anonymous`}
]}]
}
const setbot = global.db.data.settings[botNumber]
if (setbot.templateImage) {
dims.sendMessage(m.chat, { caption: menulist, image: global.png, mentions: [sender] }, { quoted: m })
} else if (setbot.templateGif) {
dims.sendMessage(m.chat, { caption: menulist + '\n\n_ketik .allmenu untuk melihat semua menu bot_', video: global.vid, gifPlayback: true, mentions: [sender] }, { quoted: m })
} else if (setbot.templateVideo) {
dims.sendMessage(m.chat, { caption: menulist + '\n\n_ketik .allmenu untuk melihat semua menu bot_', video: global.vid, mentions: [sender] }, { quoted: m })
} else if (setbot.templateMsg) {
dims.kirimListWoy(m.chat, menulist, '\nNote: Jangan di spam ya kak >,<', global.wm, data, m)
} else if (setbot.templateDocument) {
dims.send2ButtDoc(m.chat, '', menulist, global.wm, global.botname, global.media.sig, 'https://telegra.ph/file/137a5210e0723261d0d8b.jpg', 'Website', 'https://ssateam.my.id', 'Allmenu', `${prefix}allmenu`, m)        
} else if (setbot.templateButtons) {
dims.kirimBeton(m.chat, menulist, global.wm, 'Allmenu', `${prefix}allmenu`, m)
}
setTimeout( () => {
dims.sendMessage(m.chat, { react: { text: `🥶`, key: m.key }})
}, 2000)
...
if (budy.includes("@"+ dims.user.id.split('@')[0])) {
const kta = ["❓", "💤", "😦"]
const su = kta[Math.floor(Math.random() * kta.length)]
dims.sendMessage(m.chat, { react: { text: su, key: m.key }})
}

if (!m.fromMe & !m.isGroup) {
let user = global.db.data.users[m.sender]
if (new Date - user.pc < 86400000) return // setiap 24 jam sekali
let caption = `Hallo @${m.sender.split('@')[0]} ${ucapanWaktu}, Ada apa chat *${dims.user.name}*, Jika penting silakan tinggalkan pesan nanti *${dims.user.name}* akan membalas secepat mungkin.`.trim()
m.reply(caption)
user.pc = new Date() * 1
}
...
console.log(err)
dims.sendMessage(global.owner + '@s.whatsapp.net', { text: 'Hai sayang sepetinya ada yang eror, Harap di perbaiki ya!\n\n' + util.format(err) }, { quoted: m })
}
}
